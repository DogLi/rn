...
pub fn rsync<P, S>(identity: Option<P>,
             password: Option<S>,
             port: u16,
             source: &str,
             dest: &str,
             ...
             remote_ip: &str,
             delete: bool,
             exclude_files: Option<&Vec<&str>>) -> io::Result<()>
where P: AsRef<Path>, S: AsRef<str> {
    ...
    match identity {
        ...
        Some(path) => {
            ...
            login_strings = format!(r#"ssh -i {} -p {} -o "UserKnownHostsFile=/dev/null" -o "StrictHostKeyChecking no" -o "ConnectTimeout=2""#, path, port);
        }
    }
...
mod tests {
    ...
    fn test_rsync() {
        ...
        if let Err(e) = rsync(Some(id_file), password, port,source, dest, username, remote_ip, false, Some(&exclude_files)) {
